var game={
	data:null,//保存二维数组，其中保存游戏所有格子的数据
	RN:4,//保存总行数
	CN:4,//保存总列数
	score:0,//保存游戏当前的得分
	state:1,//保存游戏的状态
	...
	init:function(){		//初始化游戏的格子div
      ...
      '" class="cell"></div>';
		//计算gridPanel的宽度保存在变量width：
		var width=this.CELLSIZE*this.CN+this.OFFSET*(this.CN+1)    //CELLSIZE*CN+OFFSET*(CN+1)	
		//设置gridPanel的style的width为  width+“px”
		gridPanel.style.width=width+'px';
		//计算gridPanel的高度为height：
		var height=this.CELLSIZE*this.RN+this.OFFSET*(this.RN+1)   //CELLSIZE*RN+OFFSET*(RN+1)	
		//设置gridPanel的style的height为  height+“px”
	...
	start:function(){       //启动游戏
		...
		this.init();
		this.state=this.RUNNING;     //初始化游戏状态为  运行中
		this.score=0;      //分数清零
		//初始化二维数组
...
/***********************************生成随机数以及更新页面*************************************/
	randomNum:function(){//只负责生成一个随机数
		for(;;){//反复执行															
			var r=parseInt(Math.random()*this.RN);		//在0~RN-1之间随机生成一个行号r
			var c=parseInt(Math.random()*this.CN);	//在0~CN-1之间随机生成一个列号c
